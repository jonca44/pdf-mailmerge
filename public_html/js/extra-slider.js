 ...
 */ (function ($) {
    $.fn.bjqs_1 = function (options) {
        ...
        slides.addClass('bjqs_1-slide');

        // Phat Controller(s)
        if (settings.showControls && slideCount > 1) {

            // Create the elements for the controls
            var $controlContainer = $('<ul class="bjqs_1-controls"></ul>'),
                $next = $('<li><a href="#" class="bjqs_1-next" class="controls">' + settings.nextText + '</a></li>'),
                $previous = $('<li><a href="#" class="bjqs_1-prev" class="controls">' + settings.prevText + '</a></li>');

            // Bind click events to the controllers
            $next.click(function (e) {
                e.preventDefault();
                if (!animating) {
                    bjqs_1Go(forward, false);
                }
            });
        ...
        if (settings.showMarkers && slideCount > 1) {
            ...
            var $markerContainer = $('<ol class="bjqs_1-markers"></ol>'),
        ...
        if (settings.keyboardNav && slideCount > 1) {
            ...
            $(document).keyup(function (event) {
                ...
                if (paused & settings.automatic) {
                    bjqs_1Interval = setInterval(function () {
                        bjqs_1Go(forward)
        ...
        if (settings.useCaptions) {
            ...
            $.each(slides, function (key, value) {
                ...
                if (title) {
                    var $caption = $('<p class="bjqs_1-caption">' + title + '</p>');
        ...
        if (settings.animation === 'slide' && slideCount > 1) {
            ...
            $wrapper = $('<div class="bjqs_1-wrapper"></div>').css({
                'width': settings.width,
                'height': settings.height,
                'overflow': 'hidden',
                'position': 'relative'
        ...
        if (settings.automatic && slideCount > 1) {
            var bjqs_1Interval = setInterval(function () {
                bjqs_1Go(forward, false)
            }, settings.rotationSpeed);
        }
        ...
        var bjqs_1Go = function (direction, position) {
